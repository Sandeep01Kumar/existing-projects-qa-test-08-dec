...
> README's title and the repository are named `hao-backprop-test`. This is a known
> inconsistency that is documented here (not resolved) to avoid an unexpected rename.
...
```mermaid
graph LR
    Client[HTTP Client] -->|GET /| Root["GET / handler (server.js L32-L44)"]
    Client -->|GET /evening| Evening["GET /evening handler (server.js L46-L58)"]
    subgraph app["server.js - Express application"]
        Root --> Listener["app.listen(3000) (server.js L60-L71)"]
        Evening --> Listener
    end
    Listener --> Port["Port 3000 @ 127.0.0.1"]
```

## Prerequisites

- **Node.js `>= 18`** — required by the `express` dependency (`engines.node` = `">= 18"`).
  Node.js 20 LTS or newer is recommended. Source: `package-lock.json`
  (`node_modules/express` → `engines.node`).
...
> The `Date` header reflects the server clock and varies per response; `ETag`, `Connection`,
> and `Keep-Alive` are Express/Node defaults. The body ends with a trailing newline (`\n`),
...
- **Content-Length:** `146` (Express default error page)
